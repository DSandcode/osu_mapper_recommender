...
def appened_row(r_dict, df_row):
    r_dict["beatmapid"].append(df_row["beatmapid"])
    r_dict["beatmapsetid"].append(df_row["beatmapsetid"])
    ...
    r_dict["hpdrainrate"].append(df_row["hpdrainrate"])
    r_dict["circlesize"].append(df_row["circlesize"])
    r_dict["overalldifficulty"].append(df_row["overalldifficulty"])
    r_dict["approachrate"].append(df_row["approachrate"])
    r_dict["slidermultiplier"].append(df_row["slidermultiplier"])
    r_dict["slidertickrate"].append(df_row["slidertickrate"])
    r_dict["mode"].append(df_row["mode"])
    r_dict["mapper"].append(df_row["mapper"])
...
def make_df_row(osu_dict):
    ...
    r_row = {
        ...
        "distance": 0,
        "hitsounds": 0,
        "hpdrainrate": osu_dict["difficulty"]["hpdrainrate"],
        "circlesize": osu_dict["difficulty"]["circlesize"],
        "overalldifficulty": osu_dict["difficulty"]["overalldifficulty"],
        "approachrate": osu_dict["difficulty"]["approachrate"],
        "slidermultiplier": osu_dict["difficulty"]["slidermultiplier"],
        "slidertickrate": osu_dict["difficulty"]["slidertickrate"],
        "mode": osu_dict["general"]["mode"],
        "mapper": osu_dict["metadata"]["creator"],
    }
...
def add_rows(directory, df, log=False):
    for subdir, dirs, files in os.walk(directory):
        for filename in files:
            filepath = subdir + os.sep + filename

            if filepath.endswith(".pkl"):
                log_print(log, f"Making row for {filepath}")
                pickle_file = pickle.load(open(filepath, "rb"))

                appened_row(df, make_df_row(pickle_file))
...
if __name__ == "__main__":
    row = make_df_row(
        pickle.load(
            open(
                "data/13869387/1192164/Mili_-_String_Theocracy_([Arc])_[Advanced].pkl",
                "rb",
            )
        )
    )
